...
// dotnetBuild: Cross-platform .NET build/publish/archive
// Parameters:
...
def call(Map params = [:]) {
  ...
  def run = { String cmd ->
    isUnix() ? sh(label: cmd, script: cmd) : bat(label: cmd, script: cmd)
  }
  ...
  withEnv(['DOTNET_CLI_TELEMETRY_OPTOUT=1','DOTNET_SKIP_FIRST_TIME_EXPERIENCE=1']) {
    ...
    if (cfg.restore) {
      ...
      if (cfg.nugetConfigPath) restoreCmd.append(" --configfile ${q(cfg.nugetConfigPath)}")
      if (cfg.additionalArgs?.trim()) restoreCmd.append(' ').append(cfg.additionalArgs.trim())
      run restoreCmd.toString()
    }
    def buildCmd = new StringBuilder("dotnet build ${q(cfg.project)} -c ${cfg.configuration}")
    if (cfg.noRestoreOnBuild && cfg.restore) buildCmd.append(' --no-restore')
    if (cfg.version) buildCmd.append(" -p:Version=${cfg.version}")
    if (cfg.additionalArgs?.trim()) buildCmd.append(' ').append(cfg.additionalArgs.trim())
    run buildCmd.toString()

    if (cfg.test) {
      def testCmd = new StringBuilder("dotnet test ${q(cfg.project)} -c ${cfg.configuration} --no-build")
      if (cfg.additionalArgs?.trim()) testCmd.append(' ').append(cfg.additionalArgs.trim())
    ...
    if (cfg.publish) {
      ...
      if (cfg.runtime) publishCmd.append(" -r ${cfg.runtime}")
      if (cfg.version) publishCmd.append(" -p:Version=${cfg.version}")
      if (cfg.additionalArgs?.trim()) publishCmd.append(' ').append(cfg.additionalArgs.trim())
